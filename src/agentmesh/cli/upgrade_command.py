"""`agentmesh upgrade` — Upgrade governance level of existing config.

Adjusts the governance_level in .agentmesh.yaml from autopilot/balanced
...
def upgrade(level: str | None, config_path: str, yes: bool) -> None:
    ...
    click.echo()
    click.secho("  AgentMesh Upgrade", fg="cyan", bold=True)
    click.echo()
    ...
    if not level:
        ...
        click.echo()
        click.echo("  Usage: " + click.style("agentmesh upgrade --balanced", fg="cyan"))
        return
    ...
    header = f"# AgentMesh Configuration - governance_level: {level}\n"
    header += "# Upgraded with: agentmesh upgrade --" + level + "\n"
    ...
    # Offer to push
    click.echo("  Run: " + click.style("agentmesh push", fg="cyan") + "  to sync changes to platform")
    click.echo("  Run: " + click.style("agentmesh status", fg="cyan") + " to verify configuration")
    click.echo()
